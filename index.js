...
function setInput(v){const e=input();if(!e)return false;e.value=v;e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));e.focus();return true}
...
function send(v){if(!v||!setInput(v))return;const c=ctx();if(typeof c?.sendMessage==='function'){c.sendMessage();return}document.querySelector('#send_but')?.click()}
