...
function setInput(v,{focus=false}={}){const e=input();if(!e)return false;e.value=v;e.dispatchEvent(new Event('input',{bubbles:true}));e.dispatchEvent(new Event('change',{bubbles:true}));if(focus)e.focus();return true}
...
function render(m,cs){
 ...
 p.querySelector('.yiyi-fill').addEventListener('click',ev=>{ev.preventDefault();ev.stopPropagation();const old=input()?.value||'';setInput(compose(cs,order,old),{focus:true})});
...
function init(){
 ...
 schedule(0);console.log(EXT,'v0.2.2 loaded');
}
